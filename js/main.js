addEventListener("DOMContentLoaded", function () {
  ...
  Omega.modules.core.install([
    ...
    },
    {
      ...
      text: ["what can you do", "what can i do", "what can i do with you"],
    ...
    {
      text: ["alexa ", "siri ", "google assistant "],
      response: "Hey, I'm Omega, not something else!",
      type: "startsWith",
    },

    {
      text: ["tell me a joke", "joke me"],
      response: [
    ...
    {
      text: ["how are you", "how are you doing", "how do you do"],
      response: [
        "I'm fine.",
...

    {
      ...
      response: toElement(
        '<div class="conversation-piece fuchsia intro">' +
          ...
          '<p>You can view my source <a href="https://github.com/VanshajPoonia/omega" target="_blank">here</a>. Talk to me!</p>' +
          "</div>"
      ),
      type: "equalTo",
    },

    {
      ...
      response: function () {
        return (
          "The time is <b>" + moment().format("h:mm a").toUpperCase() + "</b>."
        );
      },
      type: "equalTo",
    },
    {
      ...
      response: function (parsed) {
        parsed = parsed.replace(
          /^(change the theme to|change theme to|set the theme to|set theme to) | /,
          ""
        );
        ...
        if (themes.includes(parsed)) {
          ...
          return (
            "Alright, I've changed the theme to " + parsed.split("_").join(" ")
          );
        } else {
          return makeConversation(
            "fuchsia",
            `<p>That's not a theme, sorry. Please try again.</p>`,
            "div"
          );
        }
      },
    ...
    },
    {
      ...
      response: function () {
        return (
          "The date is <b>" + moment().format("dddd MMMM Do YYYY") + "</b>."
        );
      },
      type: "equalTo",
    },
    ...
    {
      text: ["call me ", "my name is "],
      response: function (parsed) {
        parsed = Omega.utilities.capitalize(
          parsed.replace(/(call me)|(my name is) /, "")
        );
  ...
  Omega.createModule("web").install([
    {
      ...
      response: function (parsed) {
        ...
        return (
          'Click <a href="https://' +
          parsed +
          '" target="_blank">here</a> if your page did not automatically open.'
        );
      },
      type: "startsWith",
    },
      ...
      response: function (parsed) {
        ...
        getJSON(
          "https://api.spotify.com/v1/search?type=artist&limit=1&q=" +
            encodeURIComponent(parsed),
          function (data) {
            if (data.artists.total) {
              if (data.artists.items[0].genres) {
                genres +=
                  '<p class="artist genres"><b>Genres:</b> ' +
                  Omega.utilities.capitalize(
                    data.artists.items[0].genres.join(", ")
                  );
                ("</p>");
              }
              information.innerHTML =
                ...
                "</h1>" +
                genres +
                '<p class="artist popularity"><b>Popularity on Spotify:</b> ' +
                data.artists.items[0].popularity +
                "</p>" +
                '<p class="artist follows"><b>Followers on Spotify:</b> ' +
...

    {
      ...
      response: function (parsed) {
        ...
        parsed = parsed.replace(
          /^what is |whats |what are |whatre |what is a|/,
          ""
        );
        console.log(parsed);
        if (!isNaN(parsed.charAt(0))) {
          var endpoint = `http://api.wolframalpha.com/v2/query?input=${encodeURIComponent(
            parsed
          )}&appid=5PWTGW-3V69RYG28U&output=json`;
      ...
      response: function (parsed) {
        ...
        switch (engine) {
          case "google":
            ...
            return makeConversation(
              "fuchsia",
              "<p>Alright, I've searched Google for \"" +
                search.join(" ") +
                '".' +
                '<p>Click <a href="' +
                url +
                '" target="_blank">here</a> to open the Google Search.</p>' +
                '<div class="sources">' +
          ...
          case "soundcloud":
            var url =
              "https://www.soundcloud.com/search?q=" + search.join("%20");
            return makeConversation(
              "fuchsia",
              "<p>Alright, I've searched SoundCloud for \"" +
                search.join(" ") +
                '".' +
                '<p>Click <a href="' +
                url +
                '" target="_blank">here</a> to open the SoundCloud Search.</p>' +
                '<div class="sources">' +
          ...
          case "wolfram alpha":
            var url =
              "https://www.wolframalpha.com/input/?i=" + search.join("+");
            return makeConversation(
              "fuchsia",
              "<p>Alright, I've searched Wolfram Alpha for \"" +
                search.join(" ") +
                '".' +
                '<p>Click <a href="' +
                url +
                '" target="_blank">here</a> to open the Wolfram Alpha Search.</p>' +
                '<div class="sources">' +
          ...
          case "spotify":
            var url =
              "https://open.spotify.com/search/results/" + search.join("%20");
            return makeConversation(
              "fuchsia",
              "<p>Alright, I've searched Spotify for \"" +
                search.join(" ") +
                '".' +
                '<p>Click <a href="' +
                url +
                '" target="_blank">here</a> to open the Spotify Search.</p>' +
                '<div class="sources">' +
    ...
    },
    {
      ...
      text: ["what will the weather be in", "what is the forecast in"],
      response: function (parsed) {
        parsed = parsed.replace(/^ /, "");
        parsed = parsed.split(" ").join("+");
  ...
  Omega.fallback = function (parsed) {
    ...
    return makeConversation(
      "fuchsia",
      "<p>I'm not sure what you meant. Click <a href=\"" +
        url +
        '" target="_blank">here</a> to perform a Google search.</p>' +
        '<div class="sources">' +
        '<a href="https://www.google.com" target="_blank" class="fa fa-google" aria-hidden="true"></a>' +
        "</div>",
      "div"
